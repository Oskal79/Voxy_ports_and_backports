...
public class TextureUtils {
    ...
    public static int mipColours(boolean darkend, int C00, int C01, int C10, int C11) {
        ...
        return ColorSRGB.linearToSrgb(
                ...
                g / 4,
                b / 4,
                darkend ? ((int) a) / 4 : voxy$linearToSrgbChannel(a / 4)
        );
    }


    /**
     * 1.21.1 has no {@code net.minecraft.util.ARGB} (added in 1.21.2), so the linear -> sRGB
     * transfer function is spelled out here. Same piecewise curve the vanilla helper uses.
     */
    private static int voxy$linearToSrgbChannel(float linear) {
        float srgb = linear <= 0.0031308f
                ? linear * 12.92f
                : 1.055f * (float) Math.pow(linear, 1.0 / 2.4) - 0.055f;
        return Math.round(Math.min(Math.max(srgb, 0.0f), 1.0f) * 255.0f);
    }
}
