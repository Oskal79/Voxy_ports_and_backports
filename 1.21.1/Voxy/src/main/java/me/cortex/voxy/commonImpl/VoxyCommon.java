...
public class VoxyCommon {
    ...
    static {
        var mod = VoxyPlatform.getModInfo();
        if (mod == null) {
            ...
            var commit = VoxyPlatform.getModProperty("commit");
            // "<UnknownCommit>" is what the build writes when it is not run from a git checkout.
            boolean haveCommit = commit != null && commit.length() >= 7 && !commit.startsWith("<");
            MOD_VERSION = haveCommit ? (version + "-" + commit.substring(0, 7)) : version;
            IS_DEDICATED_SERVER = VoxyPlatform.isDedicatedServer();
            Serialization.init();
        }
    }

    /**
     * Forces this class's static initialiser to run. On Fabric that happened implicitly because
     * VoxyCommon was itself the {@code main} entrypoint; NeoForge has no equivalent, so the
     * {@code @Mod} constructor calls this instead.
     */
