...
public class DebugEntries {
    ...
    public static void onDebugText(CustomizeGuiOverlayEvent.DebugText event) {
        ...
        if (!VoxyCommon.isAvailable()) {
            left.add(ChatFormatting.RED + "voxy-" + VoxyCommon.MOD_VERSION);// installed, not available
            return;
        }

        var instance = VoxyCommon.getInstance();
        if (instance == null) {
            left.add(ChatFormatting.YELLOW + "voxy-" + VoxyCommon.MOD_VERSION);// available, no instance
            return;
        }

        VoxyRenderSystem vrs = IVoxyRenderSystemHolder.getNullable();
        left.add((vrs == null ? ChatFormatting.DARK_GREEN : ChatFormatting.GREEN) + "voxy-" + VoxyCommon.MOD_VERSION);
