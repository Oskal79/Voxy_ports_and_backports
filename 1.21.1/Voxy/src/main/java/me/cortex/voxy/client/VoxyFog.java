...
public class VoxyFog {
    ...
    public static void onRenderFog(ViewportEvent.RenderFog event) {
        ...
        if (event.getMode() == FogRenderer.FogMode.FOG_TERRAIN) {
            event.setNearPlaneDistance(RENDER_DISTANCE_PUSHED_OUT);
            event.setFarPlaneDistance(RENDER_DISTANCE_PUSHED_OUT);
        ...
        if (!VoxyConfig.CONFIG.useEnvironmentalFog && !fogIsDamnClose) {
            event.setNearPlaneDistance(PUSHED_OUT);
            event.setFarPlaneDistance(PUSHED_OUT);
            event.setCanceled(true);
        }
    }
}
