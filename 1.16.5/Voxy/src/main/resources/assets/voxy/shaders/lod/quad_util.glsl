...
uvec3 makeRemainingAttributes(const in BlockModel model, const in Quad quad, uint lodLevel, uint face) {
    ...
    //Diagnostic: keep the raw packed light index instead of the sampled light colour, so the
    //fragment shader can display the per-voxel light that was actually stored.
    attributes.x = lighting;
    #else
