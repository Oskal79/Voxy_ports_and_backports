...
vec4 computeColour(vec2 texturePos, vec4 colour) {
    #ifdef VOXY_LIGHT_DEBUG
    //Red channel = stored sky light / 15, green = stored block light / 15. A correctly lit surface
    //reads bright red day or night; a face that was stored unlit reads black.
    return vec4(float(interData.y & 0xFu) / 15.0, float((interData.y >> 4) & 0xFu) / 15.0, 0.0, 1.0);
    #endif
