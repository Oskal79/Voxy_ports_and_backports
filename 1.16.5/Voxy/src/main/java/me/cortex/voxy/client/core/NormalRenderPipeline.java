...
public class NormalRenderPipeline extends AbstractRenderPipeline {
    ...
    protected void finish(Viewport<?> viewport, int sourceDepthTexture, int outputFramebuffer, int srcWidth, int srcHeight) {
        // Bisect switch: stage 1 renders the whole LoD pipeline but never touches Minecraft's
        // colour/depth textures, so whatever remains wrong afterwards cannot be this composite.
        int debugStage = VoxyConfig.CONFIG.getDebugRenderStage();
        if (debugStage == 1) {
            return;
        }
        this.finalBlit.bind();
        boolean fogCoversAllRendering = viewport.fogParameters.environmentalEnd() < 16.0f;
        ...
        if (this.useEnvFog) {
            ...
            if (Math.abs(end-start)>1) {
                ...
                glUniform4f(4, invEndFogDelta, startDelta, net.minecraft.util.math.MathHelper.clamp(endDistance*invEndFogDelta+startDelta, 0, 1),0);//
        ...
        if (!fogCoversAllRendering) {
            ...
            boolean noDepthWrite = debugStage == 2;
            if (noDepthWrite) {
