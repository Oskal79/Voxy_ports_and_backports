...
public class MDICSectionRenderer extends AbstractSectionRenderer<MDICViewport, BasicSectionGeometryData> {
    private static int voxy$fboChecks = 0;
    /** Set when renderTerrain really issued a draw this frame; cleared by the render hook. */
    public static volatile boolean DREW_TERRAIN_THIS_FRAME = false;
    ...
    public MDICSectionRenderer(AbstractRenderPipeline pipeline, ModelStore modelStore, BasicSectionGeometryData geometryData) {
        ...
        var builder = Shader.make()
                ...
                .defineIf("VOXY_FULL_BRIGHT", me.cortex.voxy.client.config.VoxyConfig.CONFIG.debugFullBright)
                .defineIf("VOXY_LIGHT_DEBUG", me.cortex.voxy.client.config.VoxyConfig.CONFIG.debugLightView)
    ...
    private void renderTerrain(MDICViewport viewport, long indirectOffset, long drawCountOffset, int maxDrawCount) {
        ...
        // If voxy's own framebuffer failed to build, this draw lands straight in Minecraft's main
        // framebuffer using voxy's depth convention -- which would look exactly like the reported
        // "transparent world". Confirm the target is voxy's, not MC's, for the first few frames.
        if (voxy$fboChecks < 3) {
            voxy$fboChecks++;
            int boundFbo = org.lwjgl.opengl.GL11.glGetInteger(org.lwjgl.opengl.GL30.GL_DRAW_FRAMEBUFFER_BINDING);
            int mcFbo = net.minecraft.client.Minecraft.getInstance().getMainRenderTarget().frameBufferId;
            int status = org.lwjgl.opengl.GL30.glCheckFramebufferStatus(org.lwjgl.opengl.GL30.GL_DRAW_FRAMEBUFFER);
            me.cortex.voxy.common.Logger.info("[voxy-diag] terrain draw target fbo=" + boundFbo
                    + " mcFbo=" + mcFbo + " sameAsMc=" + (boundFbo == mcFbo)
                    + " complete=" + (status == org.lwjgl.opengl.GL30.GL_FRAMEBUFFER_COMPLETE)
                    + " (status=0x" + Integer.toHexString(status) + ")"
                    + " viewport=" + viewport.width + "x" + viewport.height
                    + " sections=" + this.geometryManager.getSectionCount()
        ...
        // Context-level bindings nothing else puts back: Embeddium re-binds its own indirect buffer
        // per layer, but leaving voxy's bound is a foot-gun for anything that assumes 0.
        glBindBuffer(GL_DRAW_INDIRECT_BUFFER, 0);
        glBindBuffer(GL_PARAMETER_BUFFER_ARB, 0);
        org.lwjgl.opengl.GL32.glProvokingVertex(org.lwjgl.opengl.GL32.GL_LAST_VERTEX_CONVENTION);
    ...
    public void renderOpaque(MDICViewport viewport) {
        ...
        if (me.cortex.voxy.client.config.VoxyConfig.CONFIG.getDebugStopAfter() == 12) return;   // bisect: skip voxy's draw only
