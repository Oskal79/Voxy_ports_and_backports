...
public class SoftwareRasterizer {
    ...
    private int sampleTexture(float u, float v) {
        int pu = net.minecraft.util.math.MathHelper.clamp(Math.round(u*this.samplerWidth-0.5f), 0, this.samplerWidth-1);
        int pv = net.minecraft.util.math.MathHelper.clamp(Math.round(v*this.samplerHeight-0.5f), 0, this.samplerHeight-1);
        ...
        for (int py = minY; py<=maxY; py++) {
            for (int px = minX; px<=maxX; px++) {
                ...
                if ((w1>0&&w2>0&&w3>0)||(orZero&&w1>=0&&w2>=0&&w3>=0)) {
                    ...
                    // Math.fma is Java 9+. Voxy uses it here only for the barycentric depth
                    // interpolation; a plain multiply-add differs only in the intermediate
                    // rounding, which is far below the precision this rasteriser needs.
                    float z = b1 * fromFixed(this.scratchR1.z)
                            + b2 * fromFixed(this.scratchR2.z)
                            + b3 * fromFixed(this.scratchR3.z);
    ...
    private void rasterPixel(int index, float b1, float b2, float b3, float z) {//Barry coords
        ...
        // Math.fma is Java 9+; plain multiply-add is within tolerance here (see above).
        float u = b1 * this.a1.y + b2 * this.a2.y + b3 * this.a3.y;
        float v = b1 * this.a1.z + b2 * this.a2.z + b3 * this.a3.z;
    ...
    private static int doBlending(int scr, int dst) {
        ...
        // Embeddium 0.3.18's ColorMixer only offers mixARGB(a, b, startRatio, endRatio), not the
        // newer mix(dst, src, ratio). The blend is a plain per-channel lerp, which the commented
        // original below spells out, so do it inline rather than bend the older signature.
        int blend = lerpRGB(dst, scr, dstAlpha);
    ...
    private void loadTransformPos(Matrix4f transform, long addr, int vert, Vector3f out, Vector3f otherAttributesOut) {
        ...
        out.set(maintainPrecision(((vec.x) * (0.5f) + (0.5f))*this.targetSize), maintainPrecision(((vec.y) * (0.5f) + (0.5f))*this.targetSize), vec.z);//TODO: dont know if z transform is correct
    }
    ...
    /** Per-channel {@code dst*ratio + src*(255-ratio)} over packed RGB, ignoring alpha. */
    private static int lerpRGB(int dst, int src, int ratio) {
        int inv = 255 - ratio;
        int r = (((dst      ) & 0xFF) * ratio + ((src      ) & 0xFF) * inv) / 255;
        int g = (((dst >>  8) & 0xFF) * ratio + ((src >>  8) & 0xFF) * inv) / 255;
        int b = (((dst >> 16) & 0xFF) * ratio + ((src >> 16) & 0xFF) * inv) / 255;
        return r | (g << 8) | (b << 16);
    }
}
