...
public class Serialization {
    ...
    private static final class GsonConfigSerialization <T> implements TypeAdapterFactory {
        ...
        private JsonElement serialize(Gson gson, T value) {
            ...
            // JsonObject#asMap is Gson 2.10+; Minecraft 1.16.5 ships 2.8.0. entrySet() is the
            // equivalent on both.
            for (java.util.Map.Entry<String, com.google.gson.JsonElement> e
                    : vjson.getAsJsonObject().entrySet()) {
    ...
    public static void init() {
        ...
        VoxyPlatform.forEachModRoot(root -> clazzs.addAll(collectAllClasses(root, BASE_SEARCH_PACKAGE)));
        ...
        try {
            return Files.list(base.resolve(pack.replaceAll("[.]", "/"))).flatMap(inner -> {
                if (inner.getFileName().toString().endsWith(".class")) {
                    ...
                    // A directory's file name keeps its trailing separator on a zip FileSystem,
                    // unlike on the default one. Appending it verbatim builds package names like
                    // "me.cortex.voxy.client/" and then "me.cortex.voxy.client/.Foo", which never
                    // resolve -- this is why scanning found 0 config types when reading from
                    // inside the mod jar but worked in a dev run.
                    String dirName = inner.getFileName().toString();
                    while (dirName.endsWith("/") || dirName.endsWith(java.io.File.separator)) {
                        dirName = dirName.substring(0, dirName.length() - 1);
                    }
                    return collectAllClasses(base, pack + "." + dirName).stream();
                } else {
                    return Stream.of();
                }
            }).collect(Collectors.toList());
