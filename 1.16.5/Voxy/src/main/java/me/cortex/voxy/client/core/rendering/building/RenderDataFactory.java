...
public class RenderDataFactory {
    ...
    private int presentNeighbors = 0;
    ...
    private static long packPartialQuadData(int modelId, long state, long metadata) {
        ...
        //Keep block's own light in lightAndBiome so fallback can use it if neighbor light is 0/missing
    ...
    private void acquireNeighborData(WorldSection section, int msk) {
        ...
        if ((msk&8)!=0) {//+y
            ...
            if (sec != null) {
                ...
                //Above the world or ungenerated sky: fill with open air daylight (sky light = 15)
                //so top faces of blocks facing the sky are fully lit by daylight
                long skyAir = 15L << 56;
                for (int i = 0; i < 32*32; i++) {
        ...
        if ((msk&16)!=0) {//-z
            ...
            if (sec != null) {
                this.presentNeighbors |= 16;
                var raw = sec._unsafeGetRawDataArray();
                for (int i = 0; i < 32*32; i++) {
                    this.neighboringFaces[i+32*32*4] = raw[me.cortex.voxy.common.util.Bits.expand(i,0b11111_00000_11111)|(0x1F<<5)];//pull the +z faces from the section
                }
                sec.release(WorldSection.RELEASE_HINT_POSSIBLE_REUSE);
            }
        }
        if ((msk&32)!=0) {//+z
            var sec = this.world.acquireIfExists(section.lvl, section.x, section.y, section.z + 1);
            if (sec != null) {
                this.presentNeighbors |= 32;
                var raw = sec._unsafeGetRawDataArray();
                for (int i = 0; i < 32*32; i++) {
                    this.neighboringFaces[i+32*32*5] = raw[me.cortex.voxy.common.util.Bits.expand(i,0b11111_00000_11111)];//pull the -z faces from the section
                }
                sec.release(WorldSection.RELEASE_HINT_POSSIBLE_REUSE);
            }
        }
    ...
    private void generateYZOpaqueOuterGeometry(int axis) {
        ...
        for (int side = 0; side < 2; side++) {//-, +
            int f = ((axis + 1) * 2) + side;
            if ((this.presentNeighbors & (1 << f)) == 0) {
                //Horizontal/vertical neighbor section does not exist yet; skip outer face to prevent black walls
                continue;
            }
            ...
            for (int other = 0; other < 32; other++) {
                ...
                while (msk != 0) {
...

                    {
                        ...
                        long nLight = (neighborId & (0xFFL << 56)) >>> 1;
                        if (nLight == 0) {
                            nLight = A & LM;
                        }
        ...
        for (int y = 0; y < 32; y++) {
            ...
            for (int z = 0; z < 32; z++) {
                ...
                if (z == 30 && partialHasCount != 0) {//Hackfix for incremental count overflow issue
                    ...
                    //Clear the sum
                    sumA &= ~(me.cortex.voxy.common.util.Bits.expand(Integer.toUnsignedLong(partialHasCount), X_I_MSK)*0x1F);
                    sumB &= ~(me.cortex.voxy.common.util.Bits.expand(Integer.toUnsignedLong(partialHasCount)>>11, X_I_MSK)*0x1F);
                    sumC &= ~(me.cortex.voxy.common.util.Bits.expand(Integer.toUnsignedLong(partialHasCount)>>22, X_I_MSK)*0x1F);
                }
                ...
                {//Dont need this as can just increment everything then -1 in mask
                    ...
                    sumA += me.cortex.voxy.common.util.Bits.expand(imsk, X_I_MSK);
                    sumB += me.cortex.voxy.common.util.Bits.expand(imsk>>11, X_I_MSK);
                    sumC += me.cortex.voxy.common.util.Bits.expand(imsk>>22, X_I_MSK);
                }*/
    ...
    private void generateXOuterOpaqueGeometry() {
        ...
        boolean hasNegX = (this.presentNeighbors & 1) != 0;
        boolean hasPosX = (this.presentNeighbors & 2) != 0;
    ...
    private static int voxy$neighbourReports = 0;
    private static final String[] VOXY_NEIGHBOUR_FACES = {"-x", "+x", "-y", "+y", "-z", "+z"};

    /**
     * Reports how much of each fetched neighbour face came back as a zero mapping id.
     * <p>
     * A zero entry is air carrying light 0 -- indistinguishable from a section that was never
     * populated. The mesher lights a fully-opaque block's face from the voxel on the far side, so
     * a boundary face whose neighbour reads zero is drawn unlit. Genuine open sky above terrain is
     * air with sky 15, i.e. non-zero, so a high +y zero count means the section above this one has
     * no data rather than that it is legitimately dark.
     */
    private static void voxy$censusNeighbours(WorldSection section, int msk, long[] faces) {
        if (voxy$neighbourReports >= 60) return;
        StringBuilder sb = null;
        for (int f = 0; f < 6; f++) {
            if ((msk & (1 << f)) == 0) continue;
            int zeros = 0;
            for (int i = 0; i < 32 * 32; i++) {
                if (faces[i + 32 * 32 * f] == 0) zeros++;
            }
            if (zeros == 0) continue;
            if (sb == null) sb = new StringBuilder();
            else sb.append(' ');
            sb.append(VOXY_NEIGHBOUR_FACES[f]).append('=').append(zeros);
        }
        if (sb != null && voxy$neighbourReports++ < 60) {
            me.cortex.voxy.common.Logger.info("[voxy-census] mesh lvl=" + section.lvl
                    + " sec " + section.x + "," + section.y + "," + section.z
                    + " msk=" + Integer.toBinaryString(msk) + " zeroNeighbours/1024: " + sb);
        }
    }
    ...
    public BuiltSection generateMesh(WorldSection section) {
        ...
        if (CHECK_NEIGHBOR_FACE_OCCLUSION) {
            ...
            voxy$censusNeighbours(section, neighborMsk, this.neighboringFaces);
        }
