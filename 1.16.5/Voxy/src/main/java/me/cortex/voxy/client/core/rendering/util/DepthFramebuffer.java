...
public class DepthFramebuffer {
    ...
    public void clear(float depth) {
        try (var stack = MemoryStack.stackPush()) {
            nglClearNamedFramebufferfv(this.framebuffer.id, GL_DEPTH, 0, org.lwjgl.system.MemoryUtil.memAddress(stack.floats(depth)));
        }
    }

    public void clearStencil(int to) {
        try (var stack = MemoryStack.stackPush()) {
            nglClearNamedFramebufferiv(this.framebuffer.id, GL_STENCIL, 0, org.lwjgl.system.MemoryUtil.memAddress(stack.ints(to)));
        }
    }
