...
import me.cortex.voxy.common.util.Cleaner;
...
public class VoxyRenderSystem {
    ...
    public VoxyRenderSystem(WorldEngine world, ServiceManager sm) {
        ...
        if (Minecraft.getInstance().options.renderDistance<3) {
        ...
        try {
            ...
            {
                // 1.16.5 worlds are always y=0..255, i.e. sections 0..15; getMinSection/
                // getMaxSection only arrived with the 1.18 world-height change.
                int minSec = 0 >> 5;
                int maxSec = (16 - 1) >> 5;
    ...
    private boolean checkPipelineSwap() {
        if (IrisUtil.isIrisInstalled() && IrisUtil.SHADER_SUPPORT) {
            boolean shouldBeIris = IrisUtil.isShaderPackVoxyCompatible();
            boolean needsSwap = shouldBeIris != (this.pipeline instanceof IrisVoxyRenderPipeline);
            if (!needsSwap && this.pipeline instanceof IrisVoxyRenderPipeline) {
                IrisShaderPatch activePatch = OculusPipelineBridge.findActiveShaderPatch();
                IrisVoxyRenderPipeline ivrp = (IrisVoxyRenderPipeline) this.pipeline;
                if (activePatch != ivrp.getData().patch) {
                    needsSwap = true;
                }
            }
            if (needsSwap) {
                Logger.info("Hot-swapping Voxy render pipeline (shouldBeIris=" + shouldBeIris + ")");
                this.pipeline.free();
                ...
                this.traversal.lateStageCompile(this.pipeline);
                var sectionRenderer = getRenderBackendFactory().create(this.pipeline, this.modelService.getStore(), this.geometryData);
                this.pipeline.setSectionRenderer(sectionRenderer);
                this.viewportSelector = new ViewportSelector<>(sectionRenderer::createViewport);
                return true;
            }
    ...
    public void renderOpaque(Viewport<?> viewport, int sourceDepthTexture, int sourceColourTexture) {
        ...
        // stopAfter 0..2 skips the pipeline outright, leaving only this method's own prologue and
        // the state reset below -- the narrowest step of the bisect.
        int stopAfter = VoxyConfig.CONFIG.getDebugStopAfter();
        ...
        {//Reset state manager stuffs
            ...
            org.lwjgl.opengl.GL30.glBindVertexArray(0);//Clear binding
            glBindVertexArray(0);
    ...
    private void autoBalanceSubDivSize() {
        ...
        //Auto fps targeting
        if (net.minecraft.client.Minecraft.fps < MIN_FPS) {
            VoxyConfig.CONFIG.subDivisionSize = Math.min(VoxyConfig.CONFIG.subDivisionSize + INCREASE_PER_SECOND / Math.max(1f, net.minecraft.client.Minecraft.fps), 256);
        }

        if (MAX_FPS < net.minecraft.client.Minecraft.fps && canDecreaseSize) {
            VoxyConfig.CONFIG.subDivisionSize = Math.max(VoxyConfig.CONFIG.subDivisionSize - DECREASE_PER_SECOND / Math.max(1f, net.minecraft.client.Minecraft.fps), 28);
        }
    }
    ...
    private static Matrix4f computeProjectionMat(RenderProperties properties, Matrix4fc base) {
        ...
        // 1.16.5 has no RenderSystem#getProjectionMatrix (that arrived with shader-driven
        // rendering in 1.17); the projection is still fixed-function GL state.
        java.nio.FloatBuffer projBuf = org.lwjgl.BufferUtils.createFloatBuffer(16);
        org.lwjgl.opengl.GL11.glGetFloatv(org.lwjgl.opengl.GL11.GL_PROJECTION_MATRIX, projBuf);
        var rawMCProj = new Matrix4f().set(projBuf);
