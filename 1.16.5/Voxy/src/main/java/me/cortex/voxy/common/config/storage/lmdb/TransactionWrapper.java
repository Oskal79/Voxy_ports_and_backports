...
public class TransactionWrapper {
    ...
    public TransactionWrapper put(ByteBuffer key, ByteBuffer val, int flags) {
        try (var stack = stackPush()) {
            E(mdb_put(this.transaction, this.dbi, MDBVal.mallocStack(stack).mv_data(key), MDBVal.mallocStack(stack).mv_data(val), flags));
            return this;
        }
    }
    ...
    public TransactionWrapper del(ByteBuffer key) {
        try (var stack = stackPush()) {
            E(mdb_del(this.transaction, this.dbi, MDBVal.mallocStack(stack).mv_data(key), null));
            return this;
        }
    }
    ...
    public ByteBuffer get(ByteBuffer key) {
        //TODO: instead give TransactionWrapper its own scratch buffer that it can use
        try (var stack = stackPush()) {
            var ret = MDBVal.mallocStack(stack);
            int retVal = mdb_get(this.transaction, this.dbi, MDBVal.callocStack(stack).mv_data(key), ret);
