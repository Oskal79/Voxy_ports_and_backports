...
public class BoundRenderer {
    //Clip-space depth nudge applied to the chunk mask; see chunkoutline/outline.vsh.
    //Tunable at launch with -Dvoxy.boundDepthBias=<float> for anyone chasing a seam at
    //the real-chunk/LoD border: bigger leaks LoD over real terrain, smaller risks the
    //mask under-covering by a depth-buffer ulp.
    private static final float BOUND_DEPTH_BIAS =
            Float.parseFloat(System.getProperty("voxy.boundDepthBias", "0.0005"));
    ...
    public void render(Viewport<?> viewport, IBoundStore store) {
        ...
        final float renderDistance = Minecraft.getInstance().options.renderDistance*16;//In blocks
    ...
    private void renderInner(Viewport<?> viewport, float renderDistanceBlocks, int chunkCount) {
        ...
        {//This is recomputed to be in chunk section space not worldsection
            ...
            MemoryUtil.memPutFloat(ptr, BOUND_DEPTH_BIAS); ptr += 4;
        }
