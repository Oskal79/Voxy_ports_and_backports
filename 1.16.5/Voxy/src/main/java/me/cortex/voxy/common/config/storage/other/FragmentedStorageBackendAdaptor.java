...
import me.cortex.voxy.common.util.RandomSupport;
...
public class FragmentedStorageBackendAdaptor extends StorageBackend {
    ...
    private static final class EqualingArray {
        private final byte[] bytes;

        public EqualingArray(byte[] bytes) {
            this.bytes = bytes;
        }

        public byte[] bytes() { return this.bytes; }

        @Override public String toString() { return "EqualingArray[bytes=" + this.bytes + "]"; }
    ...
    public List<StorageBackend> getChildBackends() {
        return java.util.Arrays.asList(this.backends);
    }
