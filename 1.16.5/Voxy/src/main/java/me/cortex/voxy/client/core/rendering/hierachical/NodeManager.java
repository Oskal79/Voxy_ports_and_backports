...
public class NodeManager {
    ...
    /** [voxy-diag] per-LoD-level tallies of what the world store actually hands back. */
    private static final int[] VOXY_DIAG_RESULTS = new int[me.cortex.voxy.common.world.WorldEngine.MAX_LOD_LAYER + 1];
    private static final int[] VOXY_DIAG_EMPTY = new int[me.cortex.voxy.common.world.WorldEngine.MAX_LOD_LAYER + 1];
    private static final int[] VOXY_DIAG_HASCHILDREN = new int[me.cortex.voxy.common.world.WorldEngine.MAX_LOD_LAYER + 1];
    private static final int[] VOXY_DIAG_DISCARDED = new int[me.cortex.voxy.common.world.WorldEngine.MAX_LOD_LAYER + 1];
    private static int voxy$diagTick = 0;

    public static void voxy$logSectionDiag(int topLevelNodes) {
        me.cortex.voxy.common.Logger.info("[voxy-diag] world store by LoD level (index = level):"
                + " results=" + java.util.Arrays.toString(VOXY_DIAG_RESULTS)
                + " emptyMesh=" + java.util.Arrays.toString(VOXY_DIAG_EMPTY)
                + " hasChildren=" + java.util.Arrays.toString(VOXY_DIAG_HASCHILDREN)
                + " discardedNotActive=" + java.util.Arrays.toString(VOXY_DIAG_DISCARDED)
    ...
    public void processGeometryResult(BuiltSection sectionResult) {
        ...
        {
            int lvl = me.cortex.voxy.common.world.WorldEngine.getLevel(pos);
            if (lvl >= 0 && lvl < VOXY_DIAG_RESULTS.length) {
                VOXY_DIAG_RESULTS[lvl]++;
                if (sectionResult.isEmpty()) VOXY_DIAG_EMPTY[lvl]++;
                if (sectionResult.childExistence != 0) VOXY_DIAG_HASCHILDREN[lvl]++;
                if (this.activeSectionMap.get(pos) == -1) VOXY_DIAG_DISCARDED[lvl]++;
            }
            if ((voxy$diagTick++ % 2000) == 0) {
                voxy$logSectionDiag(this.topLevelNodes.size());
            }
        }
        int nodeId = this.activeSectionMap.get(pos);
        if (nodeId == -1) {
