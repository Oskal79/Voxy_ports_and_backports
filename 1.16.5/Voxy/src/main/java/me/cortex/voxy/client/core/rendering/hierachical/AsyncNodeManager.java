...
public class AsyncNodeManager {
    ...
    static {
        try {
            RESULT_HANDLE = VarHandle.find(AsyncNodeManager.class, "results", SyncResults.class);
            RESULT_CACHE_1_HANDLE = VarHandle.find(AsyncNodeManager.class, "resultCache1", SyncResults.class);
            RESULT_CACHE_2_HANDLE = VarHandle.find(AsyncNodeManager.class, "resultCache2", SyncResults.class);
    ...
    private void run() {
        ...
        if (prev == null) {
            ...
            if (!this.tlnIdChange.isEmpty()) {//Merge top level node id changes
                var iter = this.tlnIdChange.iterator();
                while (iter.hasNext()) {
            ...
            if (!this.cleanerIdResetClear.isEmpty()) {//Merge top level node id changes
                var iter = this.cleanerIdResetClear.iterator();
                while (iter.hasNext()) {
                    int val = iter.nextInt();
    ...
    public void tick(GlBuffer nodeBuffer, NodeCleaner cleaner) {//TODO: dont pass nodeBuffer here??, do something else thats better
        ...
        if (!results.tlnDelta.isEmpty()) {
            var iter = results.tlnDelta.iterator();
            while (iter.hasNext()) {
                int val = iter.nextInt();
