...
public abstract class AbstractSectionRenderer <T extends Viewport<T>, J extends IGeometryData> {
    ...
    public static final class Factory<VIEWPORT extends Viewport<VIEWPORT>, GEODATA extends IGeometryData> {
        private final Class<? extends AbstractSectionRenderer<VIEWPORT, GEODATA>> clz;
        private final FactoryConstructor<VIEWPORT, GEODATA> constructor;

        public Factory(Class<? extends AbstractSectionRenderer<VIEWPORT, GEODATA>> clz,
                       FactoryConstructor<VIEWPORT, GEODATA> constructor) {
            this.clz = clz;
            this.constructor = constructor;
        }

        public Class<? extends AbstractSectionRenderer<VIEWPORT, GEODATA>> clz() { return this.clz; }
        public FactoryConstructor<VIEWPORT, GEODATA> constructor() { return this.constructor; }
    ...
    protected static void addDirectionalFaceTint(Shader.Builder<?> builder, ClientWorld cl) {
        // 1.21.1 has no CardinalLighting record; the same per-face multipliers come straight out of
        // World#getShade, which is what the commented-out block below already assumed.
        builder.define("NO_SHADE_FACE_TINT", cl.getShade(Direction.UP, true));
