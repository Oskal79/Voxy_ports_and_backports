...
public class WorldIdentifier {
    private static final RegistryKey<DimensionType> NULL_DIM_KEY = RegistryKey.create(Registry.DIMENSION_TYPE_REGISTRY, new ResourceLocation("voxy:null_dimension_id"));
    ...
    private static <T> boolean equal(RegistryKey<T> a, RegistryKey<T> b) {
        ...
        return a.getRegistryName().equals(b.getRegistryName()) && a.location().equals(b.location());
    }
    ...
    public static class GsonAdapter extends TypeAdapter<WorldIdentifier> {
        ...
        public WorldIdentifier read(JsonReader reader) throws IOException {
            ...
            var key = RegistryKey.create(Registry.DIMENSION_REGISTRY, new ResourceLocation(sKey));
            var dim = RegistryKey.create(Registry.DIMENSION_TYPE_REGISTRY, new ResourceLocation(sDim));
