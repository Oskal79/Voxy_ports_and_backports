...
public abstract class AbstractRenderPipeline extends TrackedObject {
    ...
    public void runPipeline(Viewport<?> viewport, int sourceDepthTexture, int sourceColourTexture, int srcWidth, int srcHeight) {
        // -Dvoxy.debugStopAfter=N truncates the pipeline after step N so the step that corrupts
        // vanilla rendering can be bisected instead of guessed at. -1 runs the whole thing.
        final int stop = me.cortex.voxy.client.config.VoxyConfig.CONFIG.getDebugStopAfter();
