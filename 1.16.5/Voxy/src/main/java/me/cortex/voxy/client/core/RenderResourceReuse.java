...
public class RenderResourceReuse {
    ...
    private static long getGeometryBufferSize() {
        ...
        // Upstream sizes this at 4 GiB and allocates it non-sparse on drivers without the nvidia
        // workaround, which simply reserves 4 GiB of VRAM for the session. Cap it to the configured
        // budget (never below the 512 MiB floor above).
        int configuredMb = me.cortex.voxy.client.config.VoxyConfig.CONFIG.geometryBufferMb;
        //Clamp rather than ignore out-of-range values: a config written before this option existed
        //deserialises as 0, which previously fell through and reserved the full 4 GiB.
        configuredMb = Math.max(512, Math.min(4096, configuredMb == 0 ? 1024 : configuredMb));
        geometryCapacity = Math.min(geometryCapacity, (long) configuredMb * 1024L * 1024L);
