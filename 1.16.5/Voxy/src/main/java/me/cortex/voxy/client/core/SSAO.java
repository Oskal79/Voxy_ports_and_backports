...
public class SSAO {
    ...
    public static SSAO createSSAO(RenderProperties properties, SSAOMode mode) {
        if (mode == SSAOMode.OFF) {
            return new SSAO(properties, false, 0, true);
        } else if (mode == SSAOMode.BASIC) {
    ...
    /** When set, the pass is a straight copy: no occlusion is computed or applied. */
    ...
    public SSAO(RenderProperties properties, boolean betterSSAO, int samples) {
        this(properties, betterSSAO, samples, false);
    }

    public SSAO(RenderProperties properties, boolean betterSSAO, int samples, boolean disabled) {
        ...
        //"off" runs the identical pass with only the occlusion multiply skipped. A plain texture
        //copy is NOT an equivalent control: this pass also resolves the alpha channel, which
        //carries per-quad metadata rather than opacity, and the composite blends on that alpha.
        builder.defineIf("SSAO_DISABLED", disabled);
    ...
    public void addDebugInfo(List<String> debugLines) {
        debugLines.add("SSAO: "+(this.disabled?"off":(this.isBetterSSAO?("new ("+this.spp+" spp)"):"basic")));
    }
}
