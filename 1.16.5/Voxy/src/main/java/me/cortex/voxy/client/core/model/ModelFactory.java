...
import me.cortex.voxy.common.util.VarHandle;
...
public class ModelFactory {
    ...
    private static final class ModelEntry {
        ...
        public ModelEntry(ColourDepthTextureData down, ColourDepthTextureData up, ColourDepthTextureData north, ColourDepthTextureData south, ColourDepthTextureData west, ColourDepthTextureData east, int fluidBlockStateId, int tintingColour) {
            this.down = down;
            this.up = up;
            this.north = north;
            this.south = south;
            this.west = west;
            this.east = east;
            this.fluidBlockStateId = fluidBlockStateId;
            this.tintingColour = tintingColour;
        }

        public ColourDepthTextureData down() { return this.down; }
        public ColourDepthTextureData up() { return this.up; }
        public ColourDepthTextureData north() { return this.north; }
        public ColourDepthTextureData south() { return this.south; }
        public ColourDepthTextureData west() { return this.west; }
        public ColourDepthTextureData east() { return this.east; }
        public int fluidBlockStateId() { return this.fluidBlockStateId; }
        public int tintingColour() { return this.tintingColour; }

        @Override public boolean equals(Object obj) {
            if (this == obj) return true;
            if (!(obj instanceof ModelEntry)) return false;
            ModelEntry o = (ModelEntry) obj;
            return java.util.Objects.equals(this.down, o.down) && java.util.Objects.equals(this.up, o.up) && java.util.Objects.equals(this.north, o.north) && java.util.Objects.equals(this.south, o.south) && java.util.Objects.equals(this.west, o.west) && java.util.Objects.equals(this.east, o.east) && this.fluidBlockStateId == o.fluidBlockStateId && this.tintingColour == o.tintingColour;
        }

        @Override public int hashCode() { return java.util.Objects.hash(this.down, this.up, this.north, this.south, this.west, this.east, this.fluidBlockStateId, this.tintingColour); }

        @Override public String toString() { return "ModelEntry[down=" + this.down + ", up=" + this.up + ", north=" + this.north + ", south=" + this.south + ", west=" + this.west + ", east=" + this.east + ", fluidBlockStateId=" + this.fluidBlockStateId + ", tintingColour=" + this.tintingColour + "]"; }
    ...
    public void setCustomBlockStateMapping(Object2IntMap<BlockState> mapping) {
        ...
        if (this.storage != null && this.storage.modelBuffer != null) {
            for (int blockId = 0; blockId < this.idMappings.length; blockId++) {
                ...
                if (modelId != -1) {
                    ...
                    int customId = (mapping != null && mapping.containsKey(state)) ? mapping.getInt(state) : 0;
                    long offset = (long) modelId * ModelStore.MODEL_SIZE + 36;
                    long ptr = UploadStream.INSTANCE.upload(this.storage.modelBuffer, offset, 4);
                    MemoryUtil.memPutInt(ptr, customId);
                }
            }
        }
    }

    private static final class BlockBake {
        private final int blockId;
        private final BlockState state;

        public BlockBake(int blockId, BlockState state) {
            this.blockId = blockId;
            this.state = state;
        }

        public int blockId() { return this.blockId; }
        public BlockState state() { return this.state; }

        @Override public boolean equals(Object obj) {
            if (this == obj) return true;
            if (!(obj instanceof BlockBake)) return false;
            BlockBake o = (BlockBake) obj;
            return this.blockId == o.blockId && java.util.Objects.equals(this.state, o.state);
        }

        @Override public int hashCode() { return java.util.Objects.hash(this.blockId, this.state); }

        @Override public String toString() { return "BlockBake[blockId=" + this.blockId + ", state=" + this.state + "]"; }
    ...
    public boolean addEntry(int blockId) {
        ...
        if (blockState.getBlock() instanceof StairsBlock sb) {
            ...
            // 1.16.5 has no Block#withPropertiesOf; copy across whatever properties the base
            // block shares with the stair state (waterlogged, facing, ...), which is what that
            // helper does on later versions.
            blockState = copySharedProperties(blockState,
                    ((me.cortex.voxy.client.mixin.minecraft.AccessorStairsBlock) sb).voxy$getBaseState());
        }
        ...
        boolean isFluid = blockState.getBlock() instanceof FlowingFluidBlock;
    ...
    public boolean processAllThings() {
        ...
        while (biomeEntry != null) {
            ...
            // 1.21.1 Registry#getOptional yields the value directly, not a Holder.Reference
            ...
            var res = this.addBiome0(biomeEntry.id, mcbiomeEntry.isPresent()?mcbiomeEntry.get():DEFAULT_BIOME);
            if (res != null) {
    ...
    private ModelBakeResultUpload processTextureBakeResult(int blockId, BlockState blockState, ColourDepthTextureData[] textureData, boolean isShaded, boolean darkenedTinting, RenderType layer) {
        ...
        voxy$censusBakedColour(blockId, blockState, textureData);
        ...
        boolean hasTint = hasTintSource(blockState);

        boolean isBiomeColourDependent = false;
        if (hasTint) {
            isBiomeColourDependent = isBiomeDependentColour(blockState);
        }

        ModelEntry entry;
        {//Deduplicate same entries
            entry = new ModelEntry(textureData, clientFluidStateId, isBiomeColourDependent||!hasTint?-1:captureColourConstant(blockState, DEFAULT_BIOME)|0xFF000000);
        ...
        if (!hasTint) {
            ...
            if (!this.biomes.isEmpty()) {
                ...
                for (var biome : this.biomes) {
                    MemoryUtil.memPutInt(clrUploadPtr, captureColourConstant(blockState, biome) | 0xFF000000); clrUploadPtr += 4;
                }
            }
        }
        uploadPtr += 4;
    ...
    private static int getBlockLightEmission(BlockState state) {
        boolean isEmissive = state.emissiveRendering(new IBlockReader() {
            @Override
            public @org.jetbrains.annotations.Nullable TileEntity getBlockEntity(BlockPos pos) {
                return null;
            }
        ...
        return net.minecraft.util.math.MathHelper.clamp(state.getLightEmission(),0,15);
    }
    ...
    private BiomeUploadResult addBiome0(int id, Biome biome) {
        ...
        for (var entry : this.modelsRequiringBiomeColours) {
            ...
            for (var biomeE : this.biomes) {
                ...
                MemoryUtil.memPutInt(clrUploadPtr, captureColourConstant(entry.right(), biomeE)|0xFF000000); clrUploadPtr += 4;
            }
        }
    ...
    /**
     * Does this state carry any tint at all?
     * <p>
     * 1.21.1 has no {@code BlockTintSource} list to inspect: {@link net.minecraft.client.color.block.BlockColors#getColor}
     * simply returns -1 when no colour handler is registered for the block, which is the same signal.
     * Fluids are not in BlockColors at all -- on NeoForge their tint comes from the fluid type.
     */
    private static boolean hasTintSource(BlockState block) {
        if (block.getBlock() instanceof FlowingFluidBlock) {
            return true;
        }
        return Minecraft.getInstance().getBlockColors()
                .getColor(block, null, null, 0) != -1;
    }
    ...
    private static int captureColourConstant(BlockState state, Biome biome) {
        var getter = new IBlockDisplayReader() {
        ...
        if (state.getBlock() instanceof FlowingFluidBlock) {
            // Forge 1.16.5 exposes fluid tint through FluidAttributes rather than
            // IClientFluidTypeExtensions (which is a NeoForge 1.20.4+ API).
            return state.getFluidState().getType().getAttributes().getColor(getter, BlockPos.ZERO);
        }
        return Minecraft.getInstance().getBlockColors().getColor(state, getter, BlockPos.ZERO, 0);
    }

    /**
     * One-shot report of how a few representative blocks resolve their colour, so "LoD looks grey"
     * can be checked against numbers instead of a screenshot taken at night.
     * <p>
     * Grass and leaves are grey in the block atlas and only become green via the biome tint, so if
     * the tint path is broken these are exactly the blocks that go grey while stone and sand look
     * fine.
     */
    public static void logColourDiagnostics() {
        String[] ids = {"grass_block", "oak_leaves", "stone", "dirt", "sand", "water"};
        var mc = Minecraft.getInstance();
        if (mc.level == null) return;
        net.minecraft.world.biome.Biome biome = mc.level.getBiome(mc.player != null
                ? mc.player.blockPosition() : BlockPos.ZERO);
        me.cortex.voxy.common.Logger.info("[voxy-diag] colour check, biome="
                + mc.level.registryAccess().registryOrThrow(net.minecraft.util.registry.Registry.BIOME_REGISTRY)
                        .getKey(biome));
        for (String id : ids) {
            try {
                var block = net.minecraft.util.registry.Registry.BLOCK
                        .get(new net.minecraft.util.ResourceLocation("minecraft", id));
                BlockState state = block.defaultBlockState();
                boolean tinted = hasTintSource(state);
                boolean biomeDep = isBiomeDependentColour(state);
                int constant = captureColourConstant(state, biome);
                me.cortex.voxy.common.Logger.info(String.format(
                        "[voxy-diag]   %-12s hasTint=%-5s biomeDependent=%-5s tint=#%06X",
                        id, tinted, biomeDep, constant & 0xFFFFFF));
            } catch (Throwable t) {
                me.cortex.voxy.common.Logger.info("[voxy-diag]   " + id + " FAILED: " + t);
            }
        }
    }

    private static boolean isBiomeDependentColour(BlockState state) {
        ...
        // Probe the colour handler: if it consults getBlockTint the flag getter above trips.
        if (state.getBlock() instanceof FlowingFluidBlock) {
            state.getFluidState().getType().getAttributes().getColor(getter, BlockPos.ZERO);
        } else {
            Minecraft.getInstance().getBlockColors().getColor(state, getter, BlockPos.ZERO, 0);
        }
        return biomeDependent[0];
    }

    private static int voxy$blackFaceReports = 0;
    private static final String[] VOXY_FACE_NAMES = {"down", "up", "north", "south", "west", "east"};

    /**
     * Flags blocks whose baked LoD faces come out opaque but black.
     * <p>
     * The LoD fragment shader multiplies the atlas sample by the light colour, so a face that bakes
     * to zero is black on screen no matter how well lit the voxel is. A modded block the software
     * bakery cannot rasterise (no static model, an entity-rendered block, a missing texture) bakes
     * exactly like this, and a run of them merges into one flat black quad.
     */
    private static void voxy$censusBakedColour(int blockId, BlockState blockState, ColourDepthTextureData[] textureData) {
        if (voxy$blackFaceReports >= 40) return;
        StringBuilder black = null;
        for (int face = 0; face < textureData.length && face < 6; face++) {
            var tex = textureData[face];
            if (tex == null) continue;
            long r = 0, g = 0, b = 0;
            int opaque = 0;
            for (int px : tex.colour()) {
                if (((px >>> 24) & 0xFF) < 8) continue;
                opaque++;
                r += (px >>> 16) & 0xFF;
                g += (px >>> 8) & 0xFF;
                b += px & 0xFF;
            }
            if (opaque < 16) continue;//an empty face is discarded by the mesher, not drawn black
            long luma = (r + g + b) / (3L * opaque);
            if (luma <= 6) {
                if (black == null) black = new StringBuilder();
                else black.append(',');
                black.append(VOXY_FACE_NAMES[face]).append('=').append(luma);
            }
        }
        if (black != null && voxy$blackFaceReports++ < 40) {
            Logger.warn("[voxy-census] block id " + blockId + " " + blockState
                    + " bakes to an opaque black face: " + black);
        }
    }
    ...
    /** Applies every property of {@code from} that {@code to}'s block also declares. */
    @SuppressWarnings({"unchecked", "rawtypes"})
    private static net.minecraft.block.BlockState copySharedProperties(
            net.minecraft.block.BlockState from, net.minecraft.block.BlockState to) {
        for (net.minecraft.state.Property property : from.getProperties()) {
