...
public class TestNodeManager {
    private static final class MemoryGeometryManager extends AbstractSectionGeometryManager {
        private static final class Entry {
            ...
            public Entry(long pos, long size) {
                this.pos = pos;
                this.size = size;
            }

            public long pos() { return this.pos; }
            ...
            @Override public boolean equals(Object obj) {
                ...
                Entry o = (Entry) obj;
                return this.pos == o.pos && this.size == o.size;
            }

            @Override public int hashCode() { return java.util.Objects.hash(this.pos, this.size); }

            @Override public String toString() { return "Entry[pos=" + this.pos + ", size=" + this.size + "]"; }
    ...
    private static boolean runTest(int ITERS, int testIdx, Set<List<StackTraceElement>> traces, boolean geoRemoval, boolean requestLimiter) {
        ...
        try {
            ...
            var trace = new ArrayList<>(java.util.Arrays.asList(e.getStackTrace()));
    ...
    public static void main1(String[] args) {
        ...
        var positions = new ArrayList<>(java.util.Arrays.stream(aa.keySet().toLongArray()).filter(k->{
            return WorldEngine.getLevel(k)!=0;
        }).sorted().mapToObj(Long::valueOf).collect(java.util.stream.Collectors.toList()));
