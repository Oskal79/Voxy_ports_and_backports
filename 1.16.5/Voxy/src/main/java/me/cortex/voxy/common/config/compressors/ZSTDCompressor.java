...
public class ZSTDCompressor implements StorageCompressor {
    private static final class Ref {
        ...
        @Override public int hashCode() { return java.util.Objects.hash(this.ptr); }

        @Override public String toString() { return "Ref[ptr=" + this.ptr + "]"; }
    ...
    private static Ref createCleanableDecompressionContext() {
        ...
        // ZSTD_d_forceIgnoreChecksum. LWJGL 3.2.2's zstd bindings do not declare the
        // experimental parameter constants, so the value from zstd.h is used directly. An
        // older zstd simply rejects the id and returns an error, which is why the result is
        // not checked -- checksum verification stays on in that case.
        nZSTD_DCtx_setParameter(ctx, 1002, 1);
        var ref = new Ref(ctx);
