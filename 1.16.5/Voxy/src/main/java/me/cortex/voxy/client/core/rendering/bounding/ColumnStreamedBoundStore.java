...
public class ColumnStreamedBoundStore implements IBoundStore {
    ...
    private static int findEmitBoundingChunks(Viewport<?> viewport, float searchDistance, int capacity, long writePtr) {
        ...
        int count = 0;
        // Embeddium 0.3.18 exposes no ChunkTracker to query for ready chunks, so there is
        // nothing to enumerate here. This store only feeds the FREX path, which stage 1 does
        // not support, so it stays empty until the Embeddium layer lands.
