...
public class Mapper {
    ...
    public long getBaseId(byte light, BlockState state, Biome biome) {
    ...
    public BlockState getBlockStateFromBlockId(int blockId) {
        var entry = this.voxy$lookupState(blockId, "getBlockStateFromBlockId");
        return entry == null ? net.minecraft.block.Blocks.AIR.defaultBlockState() : entry.state;
    }

    /**
     * Bounds-checked block-state lookup.
     * <p>
     * An out-of-range id here used to throw straight out of the ingest worker, which killed the
     * whole ingest service for the session -- LoD terrain then stopped updating and never
     * recovered. A bad id means one voxel is wrong; it must not cost the service. The id is
     * reported once with a stack trace so the source can be traced.
     */
    private StateEntry voxy$lookupState(int blockId, String where) {
        if (blockId >= 0 && blockId < this.blockId2stateEntry.size()) {
            return this.blockId2stateEntry.get(blockId);
        }
        if (voxy$badIdReports++ < 5) {
            Logger.error("[voxy-diag] " + where + ": block id " + blockId + " (0x"
                    + Integer.toHexString(blockId) + ") out of range, table size "
                    + this.blockId2stateEntry.size() + " -- treating as air",
                    new IllegalStateException("bad block id trace"));
        }
        return null;
    }

    private static int voxy$badIdReports = 0;
    ...
    public int getBlockStateOpacity(int blockId) {
        var entry = this.voxy$lookupState(blockId, "getBlockStateOpacity");
        return entry == null ? 0 : entry.opacity;
    }

    /**
     * 1.16.5 has no {@code Holder}; biomes are passed directly. Forge makes {@code Biome} a
     * registry entry here, so the identifier comes straight off the object and no dynamic
     * registry has to be threaded down into the voxelizer.
     */
    public int getIdForBiome(Biome biome) {
        String biomeId = String.valueOf(biome.getRegistryName());
    ...
    public static final class StateEntry {
        ...
        public StateEntry(int id, BlockState state) {
            ...
            if (state.getBlock() instanceof LeavesBlock) {
                ...
                this.opacity = state.getLightBlock(net.minecraft.world.EmptyBlockReader.INSTANCE, net.minecraft.util.math.BlockPos.ZERO);
            }
        }

        public byte[] serialize() {
            try {
                var serialized = new CompoundNBT();
                serialized.putInt("id", this.id);
                serialized.put("block_state", BlockState.CODEC.encodeStart(NBTDynamicOps.INSTANCE, this.state).result().get());
        ...
        public static StateEntry deserialize(int id, byte[] data, boolean[] forceResave) {
            try {
                ...
                var state = BlockState.CODEC.parse(NBTDynamicOps.INSTANCE, bsc);
                if (state.error().isPresent()) {
                    Logger.info("Could not decode blockstate, attempting fixes, error: "+ state.error().get().message());
                    bsc = (CompoundNBT) DataFixesManager.getDataFixer().update(TypeReferences.BLOCK_STATE, new Dynamic<>(NBTDynamicOps.INSTANCE,bsc),0, SharedConstants.getCurrentVersion().getWorldVersion()).getValue();
                    state = BlockState.CODEC.parse(NBTDynamicOps.INSTANCE, bsc);
                    if (state.error().isPresent()) {
                        ...
                        Logger.info("Fixed blockstate to: " + state.getOrThrow(false, err -> {}));
                        forceResave[0] |= true;
                        return new StateEntry(id, state.getOrThrow(false, err -> {}));
                    }
                } else {
                    return new StateEntry(id, state.getOrThrow(false, err -> {}));
                }
