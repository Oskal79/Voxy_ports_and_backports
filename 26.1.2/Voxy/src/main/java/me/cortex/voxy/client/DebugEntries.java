...
public class DebugEntries {
    ...
    /**
     * NeoForge keeps {@code DebugScreenEntries.register} private on purpose and hands mods
     * {@link RegisterDebugEntriesEvent} on the mod bus instead, so registration is driven from
     * {@link me.cortex.voxy.neoforge.VoxyNeoForgeClient} rather than from client init.
     * (On Fabric this was a plain {@code init()} that called the vanilla method directly, which
     * worked only because Loom applies Fabric API's access widener.)
     */
    public static void register(RegisterDebugEntriesEvent event) {
        event.register(Identifier.fromNamespaceAndPath("voxy", "version"), new DebugScreenEntry() {
            @Override
        ...
        event.register(Identifier.fromNamespaceAndPath("voxy","debug"), new VoxyDebugScreenEntry());

        event.register(GPU_DEBUG, new DebugScreenEntry() {
            @Override
